...
# CRITICAL: Define BASE_DIR at the top level
...
class DHSUDLauncher:
    ...
    def setup_ui(self):
        ...
        self.status_label = tk.Label(self.root, text="● SYSTEM OFFLINE", font=("Segoe UI", 10, "bold"), fg="#ef4444", bg="#f8fafc", pady=20)
        ...
        tk.Button(self.root, text="🌐 OPEN SYSTEM", bg="#3b82f6", command=self.open_system, **btn_style).pack(pady=5, padx=40, fill="x")
        tk.Button(self.root, text="🛑 STOP SERVER", bg="#475569", command=self.stop_server, **btn_style).pack(pady=5, padx=40, fill="x")

    def start_server(self):
        if not self.server_process:
            # Use the defined BASE_DIR
    ...
    def open_system(self):
        # Change it to localhost so the Admin automatically gets the Switch Account button
