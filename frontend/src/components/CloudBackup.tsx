...
const CloudBackup: React.FC<CloudBackupProps> = ({
  ...
  const [isGoogleConnected, setIsGoogleConnected] = useState(localStorage.getItem('google_cloud_linked') === 'true');
  ...
  return (
    <div className="w-full max-w-xl mx-auto space-y-8 animate-in slide-in-from-bottom-4 px-4 sm:px-6 py-6 box-border">
      ...
      <div className="bg-white/80 backdrop-blur-xl border border-white/60 shadow-[0_8px_30px_rgb(0,0,0,0.04)] p-6 sm:p-8 md:p-8 rounded-3xl space-y-6">
        ...
        <div className="flex flex-col gap-4">
          <button onClick={handleCloudSync} disabled={isSyncing || !connectedEmail} className="w-full py-5 bg-blue-600 text-white rounded-2xl font-black text-lg shadow-xl shadow-blue-500/20 hover:bg-blue-700 transition-all disabled:opacity-50 flex items-center justify-center gap-3 order-1">
            {isSyncing ? (
          ...
          {!isRemote ? (
            !isGoogleConnected ? (
              <button 
                  ...
                  if (!connectedEmail) {
                    ...
                    axios.post('/api/connect-google/')
                      .then(res => {
                        ...
                        localStorage.setItem('google_cloud_linked', 'true');
                ...
                className="w-full py-5 bg-white border-2 border-slate-200 text-slate-700 rounded-2xl font-black text-lg hover:bg-slate-50 transition-all flex items-center justify-center gap-3 order-2"
              >
              ...
              <div className="w-full py-5 bg-emerald-50 border-2 border-emerald-100 text-emerald-700 rounded-2xl font-black text-lg flex items-center justify-center gap-3 animate-in zoom-in-95 duration-300 order-2">
            ...
            <div className="w-full py-4 px-6 bg-amber-50 border-2 border-amber-200 rounded-2xl flex items-start gap-3 order-2">
      ...
      <div className="bg-white p-6 sm:p-8 rounded-[32px] shadow-sm border border-slate-200 space-y-6">
        ...
        <button 
          ...
          disabled={isUploadingFile || !uploadFile || !connectedEmail} 
